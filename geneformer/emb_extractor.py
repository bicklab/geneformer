...
def get_embs(
    ...
    for i in trange(0, total_batch_length, forward_batch_size, leave=(not silent)):
        ...
        print("total_batch_length: ", total_batch_length)
        ...
        # # Convert input_ids to long if they're not already
        # if input_data_minibatch.dtype != torch.long:
        #     input_data_minibatch = input_data_minibatch.long()
        ...
        print("++++++++++++++++++ input_data_minibatch.shape +++++++++++++++++++: ", input_data_minibatch.shape)

        # Print current and peak GPU memory usage in GB
        current_memory = torch.cuda.memory_allocated(device) / (1024 ** 3)  
        reserved_memory = torch.cuda.max_memory_reserved(device) / (1024 ** 3)
        peak_memory = torch.cuda.max_memory_allocated(device) / (1024 ** 3)  
        total_memory = torch.cuda.get_device_properties(device).total_memory / (1024 ** 3)  

        print(f"Current GPU memory usage: {current_memory:.3f} GB, Reserved: {reserved_memory:.3f} GB, Peak: {peak_memory:.3f} GB, Total: {total_memory:.3f} GB")

        # set_trace()
        with torch.no_grad(): outputs = model(input_ids=input_data_minibatch.to("cuda").type(torch.int64),attention_mask=pu.gen_attention_mask(minibatch),)
        memory_after = torch.cuda.memory_allocated()
        memory_diff = (memory_after - memory_before) / (1024 ** 3)  

        print(f"Iteration {i // forward_batch_size}: Memory used by model: {memory_diff:.3f} GB")
        # print("dir(outputs): ", dir(outputs))   # get the output attributes
        print("outputs.logits.shape: ", outputs.logits.shape)

        # print currently alive Tensors and Variables
        for obj in gc.get_objects():
            try:
                if torch.is_tensor(obj) or (hasattr(obj, 'data') and torch.is_tensor(obj.data)):
                    # print(type(obj), obj.size())
                    # Attempt to retrieve the 'name' attribute
                    name = getattr(obj, 'name', None)
                    # print(f"Type: {type(obj)}, Size: {obj.size()}, ID: {id(obj)}", f", Name: {name}" if name else "")
            except:
                pass

        embs_i = outputs.hidden_states[layer_to_quant].cpu()
        print("embs_i.shape: ", embs_i.shape)
        print("embs_i.device: ", embs_i.device)
        print("original_lens.device: ", original_lens.device)
        # set_trace()
        print("emb_mode: ", emb_mode)
        if emb_mode == "cell":
            mean_embs = pu.mean_nonpadding_embs(embs_i, original_lens.cpu())
            if summary_stat is None:
                print("summary_stat is None")
                embs_list.append(mean_embs)
            elif summary_stat is not None:
                print("summary_stat is not None")
            ...
            if summary_stat is None:
                ...
                #set_trace()
                print("len(embs_list): ", len(embs_list))
            elif summary_stat is not None:
...
class EmbExtractor:
    ...
    def extract_embs(
        ...
        **Examples:**
        ...
        print("self.emb_mode from emb_extractor L597: ", self.emb_mode)  
        embs = get_embs(
            model,
            downsampled_data,
            self.emb_mode,
        ...
        print("self.emb_mode from emb_extractor L608: ", self.emb_mode)  
    ...
    def get_state_embs(
        ...
        **Outputs**
        ...
        print("----------------- cell_states_to_model.items() ------------------------: ", cell_states_to_model.items())
        for k, v in cell_states_to_model.items(): # runs as many number of start, end and alt states
            if k == "state_key":
                continue
