...
class InSilicoPerturber:
    ...
    def perturb_data(
        ...
        **Parameters:**
        ...
        print("self.perturb_group: ", self.perturb_group)
    ...
    def isp_perturb_set(
        ...
        def make_group_perturbation_batch(example):
            ...
            # example["input_ids"] = [int(id) for id in example["input_ids"]]
    ...
    def isp_perturb_all(
        ...
        if self.emb_mode == "cell_and_gene":
            stored_gene_embs_dict = defaultdict(list)
        print("filtered_input_data", filtered_input_data)
        for i in trange(len(filtered_input_data)):
            print("*************************** i : ", i, "of ", len(filtered_input_data), " cells for which gene embeddings are being calculated")
            ...
            if self.cell_states_to_model is not None:
                ...
                print("perturbation_cell_emb.device: ", perturbation_cell_emb.device)
                print("original_cell_emb.device: ", original_cell_emb.device)
                cell_cos_sims = pu.quant_cos_sims(
                    perturbation_cell_emb,
                    original_cell_emb,
                    self.cell_states_to_model,
