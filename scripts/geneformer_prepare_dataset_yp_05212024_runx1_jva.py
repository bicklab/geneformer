...
def reformat_loom(ds, filename):
    ...
    col_attrs_df = pd.DataFrame.from_dict(ds.ca)
    col_attrs_df = col_attrs_df.dropna(subset=['stim_status', 'label'])
    valid_indices = col_attrs_df.index
    ds_filtered = ds[:, valid_indices]

    col_attrs = col_attrs_df.to_dict('list')
    col_attrs['n_counts'] = col_attrs['nCount_RNA']
    col_attrs['organ_major'] = len(col_attrs['n_counts'])*['BM']
    #col_attrs['label'] = col_attrs['imputed_cell_label_final']
    col_attrs['label'] = col_attrs['CRISPR_target']
    #col_attrs['label'] = col_attrs['stim_status']
...
def main():
    ...
    tk = TranscriptomeTokenizer({"organ_major": "organ", "n_counts": "n_counts", "label": "label", "CellID": "CellID", "Set": "Set", "stim_status": "stim_status"}, nproc=16)
